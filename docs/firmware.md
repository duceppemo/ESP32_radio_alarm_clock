...
| [`src/MenuSystem.*`](../src/MenuSystem.h) | Debounced short/long-press handling for the three onboard buttons (D0/D1/D2) drives a Home / Alarms / Alarm-edit / Radio / WiFi-info / Set-Time screen state machine on the built-in TFT. Short press selects/confirms, long press backs out a level; on Home while an alarm is ringing, the same two gestures snooze/dismiss it. Shows battery percent and (when active) the sleep timer's live countdown on Home, and the sleep timer on the Radio screen too. Set Time writes directly to the RTC (`rtc->adjust()`) — the only manual way to correct the clock without WiFi/NTP. |
