...
| `test/test_radio_tuner/` | `begin()` reporting `available()` correctly whether or not the chip responds on I2C, FM-band clamping, volume clamping, the transient-vs-persisted volume setter, preset store/recall, sleep timer expiry and its remaining-minutes math, `setSleepTimer(0)` behaving like a cancel rather than an immediately-past deadline, persistence across instances. |
| `test/test_wake_controller/` | The sunrise volume ramp's math, the dead-air RSSI fallback (and that it does *not* fire with a good signal), beep/chime wake sources muting the radio and starting the right tone immediately, what dismissing/snoozing does and doesn't restore, and that a dismiss is noticed by `tickFast()` alone (no `tickSlow()` call at all) so it can't be delayed by `tickSlow()`'s 1 Hz throttle. |
...
- `SI4735.h` — records what `RadioTuner` sets and lets tests control what "the chip" reports back, notably `SI4735::setSimulatedRssi()` for dead-air testing and `SI4735::setSimulatedPresent()` for simulating the module not being wired up at all.
...
| [`src/MenuSystem.*`](../src/MenuSystem.h) | Debounced short/long-press handling for the three onboard buttons (D0/D1/D2) drives a Home / Alarms / Alarm-edit / Radio / WiFi-info / Set-Time / Timezone screen state machine on the built-in TFT. Short press selects/confirms; long press backs out a level, firing the instant the hold crosses `kLongPressMs` (1000ms) while the button is still down (not on release), latched so continuing to hold through the resulting screen change can't cascade into a second long press there. On Home while an alarm is ringing, the same two gestures snooze/dismiss it. Shows battery percent and (when active) the sleep timer's live countdown on Home, and the sleep timer on the Radio screen too. Set Time writes directly to the RTC (`rtc->adjust()`) — the only manual way to correct the clock without WiFi/NTP. Timezone cycles through `TimezoneStore`'s list with up/down, no WiFi needed either. D0 is active-low (external pull-up, this board's convention for every other button too); D1/D2 are active-high (external pull-down) -- `DebouncedButton`'s `activeHigh` flag accounts for the difference. Renders into an off-screen `GFXcanvas16` and blits the finished frame in one `drawRGBBitmap()` call per redraw, rather than drawing straight to the display, so the once-a-second Home clock tick doesn't visibly flash. |
| [`src/WebDashboard.*`](../src/WebDashboard.h) + [`src/DashboardPage.h`](../src/DashboardPage.h) | Implements the web dashboard described below. Every route handler takes a `StateLock` (see below) before touching any shared module. |
| [`src/StateLock.h`](../src/StateLock.h) | An RAII guard around one FreeRTOS mutex, protecting `AlarmClock`/`RadioTuner`/`TimezoneStore`/`BatteryMonitor` (and `WebDashboard`'s own `apMode_`/credentials/`restartAtMs_`) from a real cross-task race: `WebDashboard`'s route handlers run on AsyncTCP's own `async_tcp` task, not `loop()`'s task, and nothing else in this codebase synchronizes between the two. One coarse lock rather than a lock per module or a command queue — see the header's own comment for why. Taken once at the top of `loop()` and once at the top of each route handler; everything else those call into inherits whichever one is already held, so `AlarmClock`/`RadioTuner`/etc. themselves need no changes. Not used natively — `main.cpp`/`WebDashboard.cpp` aren't part of the native test build, and FreeRTOS isn't available there anyway. |
| [`src/main.cpp`](../src/main.cpp) | Owns one instance of each module, plus a `DebouncedButton` each for the snooze and two volume buttons. `MenuSystem::update()`, `WebDashboard::update()`, `WakeController::tickFast()` (keeps a playing tone fed, and notices a dismiss/snooze immediately rather than waiting for the next 1 Hz tick), and all three panel buttons run every `loop()` iteration for responsiveness; RTC reads, alarm evaluation, `RadioTuner::update()` (sleep timer expiry), and `WakeController::tickSlow()`'s ramp/dead-air timing are throttled to 1 Hz to limit I2C traffic. Also corrects `rtc`'s availability on both `menu`/`dashboard` after `rtc.begin()` reports failure — see `setRtcAvailable()`. Holds a `StateLock` for the entire `loop()` body. |
